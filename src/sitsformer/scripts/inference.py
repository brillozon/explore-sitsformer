...
from sitsformer.utils import (
    load_config, get_device, setup_logging, 
    get_system_info, set_seed
)
...
def parse_args():
    ...
    parser.add_argument("--output-format", type=str, choices=["json", "csv", "numpy"],
                       default="json", help="Output format for predictions")
...
def load_model_and_config(checkpoint_path, config_path=None):
    ...
    if 'config' in checkpoint and config_path is None:
        ...
        raise ValueError("No configuration found. Please provide --config or use checkpoint with saved config.")
...
def create_inference_dataset(config, args):
    ...
    if args.dummy_data or not args.input:
        ...
        raise NotImplementedError("Real dataset loading not implemented yet. Use --dummy-data flag.")
            ...
            'predictions': [
                {
                    ...
                    'predicted_class_name': class_names[pred] if pred < len(class_names) else f"Unknown_{pred}",
...
def main():
    ...
    logger.info(f"  Average time per sample: {results['inference_time']/results['total_samples']:.4f}s")
    
    # Class distribution
    class_names = config.get('evaluation', {}).get('class_names', [f"Class_{i}" for i in range(config['model']['num_classes'])])
    class_counts = {}
    for pred in results['predictions']:
        class_name = class_names[pred] if pred < len(class_names) else f"Class_{pred}"
